...
const Primary2ButtonStyle = styled.button`
  ...
  border: ${(props) => props.theme.border.regular}
    ${(props) => props.theme.colors.primary.blue};
  color: ${(props) => props.theme.colors.primary.blue};
  &:active {
    background-color: ${(props) =>
      props.theme.colors.primary.lightBlue_pressed};
  }
`;

export const PrimaryButton = React.memo((props: ButtonProps) => {
  return (
    <PrimaryButtonStyle onClick={props.onClick} type={props.type}>
      {props.content}
    </PrimaryButtonStyle>
  );
});
...
export const SecondaryButton = React.memo((props: ButtonProps) => {
  return (
    <SecondaryButtonStyle onClick={props.onClick} type={props.type}>
      {props.content}
...
export const Primary2Button = React.memo((props: ButtonProps) => {
  return (
    <Primary2ButtonStyle onClick={props.onClick} type={props.type}>
      {props.content}
    </Primary2ButtonStyle>
  );
});
